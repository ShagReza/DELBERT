...
from train_largedata_shay import (
    ...
    MLM_PROBABILITY, MLM_VAL_FRACTION,
...
def make_classification_loaders(train_df, test_df, tokenizer, label_col,
                                token_format, device):
    """Train uses DistributedSampler; val/test are plain (rank-0 only iterates them)."""
...
def train_classifier_loop(model, train_loader, val_loader, test_loader,
...
def run_from_scratch(device, autocast_dtype):
    ...
    train_classifier_loop(
        model, train_loader, val_loader, test_loader, train_sampler,
